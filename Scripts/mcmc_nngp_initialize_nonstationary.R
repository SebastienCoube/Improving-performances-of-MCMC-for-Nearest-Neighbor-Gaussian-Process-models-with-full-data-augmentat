mcmc_nngp_initialize_nonstationary = 
  function(observed_locs, #spatial locations
           ...
           covfun = "exponential_isotropic", response_model = "Gaussian", # covariance model and reponse model
           X_noise_obs = NULL, X_noise_locs = NULL, noise_range = NULL, # range for latent field of parameters, if NULL no latent field
           X_scale = NULL, scale_range = NULL, # range for latent field of parameters, if NULL no latent field
           X_range = NULL, range_range = NULL, # range for latent field of parameters, if NULL no latent field
  ...
  )
  {
    ...
    if(covfun == "exponential_isotropic") shape_params = c("log_range")
    if(covfun == "exponential_sphere") shape_params = c("log_range")
    if(covfun == "exponential_scaledim") shape_params = paste("log_range", seq(ncol(locs)), sep = "_")
    if(covfun == "exponential_spacetime") shape_params = c("log_range_1", "log_range_2")
    if(covfun == "exponential_nonstationary_isotropic") shape_params = c("log_range_vector")
    if(covfun == "exponential_nonstationary_isotropic_sphere") shape_params = c("log_range_vector")
    if(covfun == "exponential_nonstationary_anisotropic") shape_params = c("log_range_chol_1", "log_range_chol_2", "log_range_chol_3")
    if(covfun == "exponential_nonstationary_anisotropic_sphere") shape_params = c("log_range_chol_1", "log_range_chol_2", "log_range_chol_3")

    space_time_model$covfun$covfun = covfun
    ...
    if(!is.null(X_sc$X))
    {
      X_sc$X = model.matrix(~., X$X)
      cnames =  colnames(X_sc$X)[-1]
      X_sc$X = matrix(X_sc$X[,-1], nrow = nrow(X_sc$X))
      colnames(X_sc$X) = cnames
      X_sc$X_mean = c()
      for(i in seq(ncol(X_sc$X)))X_sc$X_mean = c(X_sc$X_mean,  mean(X_sc$X[,i])) 
      for(i in seq(ncol(X_sc$X)))X_sc$X[,i] = X_sc$X[,i] - mean(X_sc$X[,i])
      X_sc$solve_XTX = solve(crossprod(X_sc$X))
      X_sc$chol_solve_XTX = chol(X_sc$solve_XTX)
    }
    ...
    if(response_model == "Gaussian")
    {
      ...
      for(i in seq(n_chains))
      {
        ...
        states[[i]]$transition_kernels$covariance_params_sufficient$logvar = -4
        states[[i]]$transition_kernels$covariance_params_ancillary$logvar = -4
        states[[i]]$transition_kernels$log_noise_variance$logvar = -3
        states[[i]]$transition_kernels$epsilon_mala = .01
        ...
        states[[i]]$params$log_scale = .5  * log(var(naive_ols$residuals)) * (.5 + rbeta(1, shape1 = .5, shape2 = .5))
        states[[i]]$params$log_noise_variance = rbeta(1, shape1 = .5, shape2 = .5) * log(var(naive_ols$residuals))
        #field
        ...
        Linv = 
          Matrix::sparseMatrix(
            ...
            triangular = T)
        states[[i]]$params$field  = states[[i]]$params$beta_0 + sqrt(.5 * states[[i]]$params$log_scale) * as.vector(Matrix::solve(Linv, rnorm(vecchia_approx$n_locs)))
      }
    }
